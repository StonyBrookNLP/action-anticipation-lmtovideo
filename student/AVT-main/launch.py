...
def choose_single_run(clis, fpath, run_id):
    ...
    if len(run_id_param_dicts) == 1:
        ...
        #                                               ['r']]
        final_run_id, param_dict = run_id_param_dicts[0]  #custominfo here
...
def construct_cmd(args):
    ...
    if args.debug:
        if args.test:
            ...
            agent_folder = os.path.join(agent_folder, 'local_f5_roberta') # custom top k
        else:
            agent_folder = os.path.join(agent_folder, 'local_egtea_notpretrained_distillbert_kldiv_16may_wt150_t10')
