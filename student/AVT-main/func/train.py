...
def train_one_epoch(
    ...
    for i, data in enumerate(
        ...
        # else:
        #     batch_loss.append(loss.reshape((1,)))
        #     loss = torch.mean(torch.cat(batch_loss))
        #     print("batch loss mean: ", loss)
        #     batch_loss = []
        #
        #     optimizer.zero_grad()
        #     loss.backward()
        ...
        optimizer.step()
        # logger.info('custominfo: batch %d: loss: %0.8f, lr: %0.8f', i, loss.item(), optimizer.param_groups[0]['lr'])  # custom
...
def main(cfg):
    ...
    params = []
    print("custominfoLR cfg opt lr_wd", cfg.opt.lr_wd)  # custominfo
    for this_module_names, this_lr, this_wd in cfg.opt.lr_wd:
        ...
        print("custominfoLR this_lr", this_lr)  # custominfo
        this_scaled_lr = this_lr * dist_info['world_size']
        print("custominfoLR this_scaled_lr", this_scaled_lr)  # custominfo
    ...
    if os.path.isfile(last_saved_ckpt):
        ...
        print("custominfoLR lr_scheduler orig", checkpoint['lr_scheduler'])  # custominfo
        # checkpoint['lr_scheduler']['base_sched_dict']['T_max'] = 500000  # custominfo: to avoid 0 lr
        lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
        print("custominfoLR lr_scheduler 1", checkpoint['lr_scheduler'])  # custominfo
    ...
    for epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        print("custominfoLR lr_scheduler 2", lr_scheduler.state_dict())  # custominfo
        # custom_state_dict = lr_scheduler.state_dict()
        # custom_state_dict['base_sched_dict']['eta_min'] = 0.0000002
        # custom_state_dict['base_sched_dict']['_last_lr'] = [1.6e-05, 1.6e-05]
        # lr_scheduler.load_state_dict(custom_state_dict)
        # print("custominfoLR lr_scheduler 3", lr_scheduler.state_dict())  # custominfo
