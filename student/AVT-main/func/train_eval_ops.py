...
from loss_fn.sim import CosineSim
...
class BasicLossAccuracy(nn.Module):
    ...
    def forward(self, outputs, target, target_subclips):
        ...
        for tgt_type, tgt_val in target.items():
            ...
            accTopK = utils.accuracy(logits,
                                     tgt_val,
                                     topk=range(1, min(11, dataset_max_classes)))
            ...
            for k in range(1, min(11, dataset_max_classes)):
                accuracies[f'acc{k}/{tgt_type}'] = accTopK[k-1]
...
class Basic:
    def __init__(self,
        ...
        self.distillation_feat_loss = CosineSim(device)  # custom distill feat
        self.distillation_feat_loss_mse = NormedMSE()  # custom distill feat
        # # MULTI TEACHER CUSTOM
        # num_heads = 4
        # teach_feat_dim = 768
        # embed_dim = 768
        # self.multi_teacher_attention = torch.nn.MultiheadAttention(embed_dim=embed_dim, num_heads=num_heads,
        #                                                            kdim=teach_feat_dim,
        #                                                            vdim=teach_feat_dim,
        #                                                            device=device)
        # # MULTI TEACHER CUSTOM
    ...
    def __call__(
        ...
        # # MULTI TEACHER CUSTOM
        # if train_mode and 'ens_pred' in data.keys() and 'ens_feat' in data.keys():
        #     teacher_feat = data['ens_feat'].to(self.device)
        #     teacher_pred = data['ens_pred'].to(self.device)
        #     student_transformed = outputs['future_agg'].unsqueeze(dim=1).transpose(0, 1)
        #     attn_output, attn_output_weights = self.multi_teacher_attention(student_transformed,
        #                                                                     teacher_feat.transpose(0, 1),
        #                                                                     teacher_feat.transpose(0, 1))
        #     # Weighted sum of teacher predictions by the attention weights
        #     teacher_ens_pred = teacher_pred * attn_output_weights.transpose(1, 2)
        #     teacher_ens_pred = teacher_ens_pred.sum(dim=1)
        #     outputs['teacher_ens_pred'] = teacher_ens_pred
        # # MULTI TEACHER CUSTOM
        #
        # # custom multi teacher distill loss
        # if train_mode and 'teacher_ens_pred' in outputs.keys():
        #     lm_pred = outputs['teacher_ens_pred']
        #     output = outputs['logits/action']
        #     ens_distillation_feat_loss = self.distillation_loss(output, lm_pred)
        #     losses['ens_distill'] = ens_distillation_feat_loss

        # custom distill feat
        if train_mode and 'lm_pred_feat' in data:
            lm_pred = data['lm_pred_feat']
            output = outputs['future_agg']
            distillation_feat_loss = self.distillation_feat_loss(output, lm_pred)
            losses['distill_feat'] = distillation_feat_loss

        # custom distill feat mse
        if train_mode and 'lm_pred_feat' in data:
            lm_pred = data['lm_pred_feat'].to(self.device)
            output = outputs['future_agg']
            distillation_feat_loss_mse = self.distillation_feat_loss_mse(output, lm_pred)
            losses['distill_feat_mse'] = distillation_feat_loss_mse
